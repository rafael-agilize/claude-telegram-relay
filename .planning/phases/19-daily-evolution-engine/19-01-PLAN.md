...
must_haves:
  truths:
    ...
    - "Evolution tick only fires during active hours and skips if already running"
  artifacts:
    - path: "src/relay.ts"
...
1. `async function getLast24hMessages(): Promise<Array<{ role: string; content: string; thread_name: string; created_at: string }>>` — Queries `thread_messages` table for ALL rows where `created_at >= now() - 24h`. Join with `threads` table to get thread name for context. Order by `created_at ASC`. Use supabase-js `.gte('created_at', cutoffISO)` filter where cutoffISO is `new Date(Date.now() - 24*60*60*1000).toISOString()`. Select `role, content, created_at` from thread_messages and join thread name via a second query or inline select. If Supabase doesn't support joins easily, do a two-step: (a) fetch all threads, (b) fetch messages with thread_id, then map thread names. Return empty array on error (graceful fallback). Cap at 200 messages to avoid token explosion (`.limit(200)`).

2. `async function getSoulHistory(limit: number = 3): Promise<SoulVersion[]>` — Calls `supabase.rpc('get_soul_history', { p_limit: limit })`. Returns array of SoulVersion objects (reuse existing interface). Default limit 3 (not 7 — we only need recent context for reflection, not a full week). Return empty array on error.
